...
rgstring="@RG\tID:NA12878\tSM:gcat_set_041-RTG\tPL:ILLUMINA"
...
cmd="java -Xmx${maxmem} -jar $PICARD/picard.jar ValidateSamFile \
	I=${outfolder}/${rtgpe}.bam \
	O=${outfolder}/${rtgpe}-mapping_ValidateSam.txt \
	M=SUMMARY \
	VALIDATION_STRINGENCY=LENIENT"
...
cmd="samtools flagstat \
	${outfolder}/${rtgpe}.bam \
	>${outfolder}/${rtgpe}-mapping_flagstat.txt"
...
cmd="samtools stats \
	${outfolder}/${rtgpe}.bam \
	>${outfolder}/${rtgpe}-mapping_stats.txt"
...
cmd="plot-bamstats \
	${outfolder}/${rtgpe}-mapping_stats.txt \
	-p ${outfolder}/${rtgpe}-bamstats_plots/rtg"
...
cmd="java -Xmx${maxmem} -jar $PICARD/picard.jar QualityScoreDistribution \
	...
	I=${outfolder}/${rtgpe}.bam \
	O=${outfolder}/${rtgpe}-mapping_QS.txt \
	VALIDATION_STRINGENCY=LENIENT"
