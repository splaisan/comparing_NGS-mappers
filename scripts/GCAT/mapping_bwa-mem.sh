...
rgstring='@RG\tID:NA12878\tLB:lib-NA12878\tPU:unkn-0.0\tPL:ILLUMINA\tSM:gcat_set_041-BWA.mem'
...
cmd="java -Xmx${maxmem} -jar $PICARD/picard.jar ValidateSamFile \
	I=${outfolder}/${bwape}.bam \
	O=${outfolder}/${bwape}-mappings_ValidateSam.txt \
	M=SUMMARY"
...
cmd="samtools flagstat \
	${outfolder}/${bwape}.bam \
	>${outfolder}/${bwape}-mappings_flagstat.txt"
...
cmd="samtools stats \
	${outfolder}/${bwape}.bam \
	>${outfolder}/${bwape}-mappings_stats.txt"
...
cmd="plot-bamstats \
	${outfolder}/${bwape}-mappings_stats.txt \
	-p ${outfolder}/${bwape}-bamstats_plots/bwa"
...
cmd="java -Xmx${maxmem} -jar $PICARD/picard.jar QualityScoreDistribution \
	...
	I=${outfolder}/${bwape}.bam \
	O=${outfolder}/${bwape}-mappings_QS.txt"
