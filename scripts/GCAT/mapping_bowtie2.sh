...
cmd="java -Xmx${maxmem} -jar $PICARD/picard.jar ValidateSamFile \
	I=${outfolder}/${bowpe}.bam \
	O=${outfolder}/${bowpe}-mappings_ValidateSam.txt \
	M=SUMMARY"
...
cmd="samtools flagstat \
	${outfolder}/${bowpe}.bam \
	>${outfolder}/${bowpe}-mappings_flagstat.txt"
...
cmd="samtools stats \
	${outfolder}/${bowpe}.bam \
	>${outfolder}/${bowpe}-mappings_stats.txt"
...
cmd="plot-bamstats \
	${outfolder}/${bowpe}-mappings_stats.txt \
	-p ${outfolder}/${bowpe}-bamstats_plots/bowtie"
...
cmd="java -Xmx${maxmem} -jar $PICARD/picard.jar \
	...
	I=${outfolder}/${bowpe}.bam \
	O=${outfolder}/${bowpe}-mappings_QS.txt"
